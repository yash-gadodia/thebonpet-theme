...
## Before coding web-component or embedded-app integrations
1. Research the component via Shopify devdocs / context7 / WebFetch FIRST. Don't guess.
2. Write a Playwright reproduction test with a REAL `.click()` before touching code. If the test can't reproduce the user's bug, you don't understand the bug yet.
3. Check `document.elementFromPoint(x, y)` at any button you're trying to trigger - if it returns an unexpected element (e.g. `SHOPIFY-FORMS-EMBED`), there's an overlay interception and CSS-only fixes will fail.
4. `page.evaluate(() => window.someFn())` in Playwright bypasses the click pipeline. Use it for diagnostics ONLY, never for "this button works" assertions.
